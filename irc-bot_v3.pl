...
use Net::IRC ;
# On utilise la libraire Mime pour décoder la chaine de caractère
use MIME::Base64;
...
sub on_private
{
    ...
    if($time==0) 
    {
    	$text =~ tr/abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ/nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM/;
    	$conn->privmsg('Candy', "!ep3 -rep $text");
    }
    else
    {
